...
/**
 * The parts of one project — a WBS, a phase, a package — each with the hours
 * rolled up into it.
 *
 * The word comes from the project's OWN parts (their level_label), not from a
 * setting: one project is broken into a WBS and the next into phases. Before
 * the first part exists there is nothing to inherit, so the link reads "Break
 * into parts" and the name is chosen on the form.
 */
export default function SubItems({ projectId }: { projectId: string }) {
  ...
  // Already as deep as anything may go — nothing to add, nothing to show.
  ...
  // Whatever this project's existing parts are already called. The first one
  // names them; every later one inherits the word.
  const named = children.length > 0;
  const childLevel = childLabelFrom(children.map((ch) => ch.level_label));
    ...
    <section style={{ ...cardStyle, padding: 18, marginTop: 16 }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap" }}>
        <span style={{ fontSize: 15, fontWeight: 700, color: c.ink }}>
          {named ? childLevel : "Parts of this project"}
        </span>
        ...
        <Link
          href={`${ROUTES.wfmProjectNew}?parent=${projectId}${named ? `&label=${encodeURIComponent(childLevel)}` : ""}`}
          style={{
            marginLeft: "auto", fontSize: 12.5, fontWeight: 600, color: c.accent,
            textDecoration: "none", whiteSpace: "nowrap",
          }}
        >
          {named ? `+ Add ${childLevel}` : "+ Break into parts"}
        </Link>
      </div>

      {children.length === 0 ? (
        <div style={{ fontSize: 12.5, color: c.hint, marginTop: 12, lineHeight: 1.5 }}>
          Not broken into parts. You don&apos;t have to — hours can sit on the project itself.
          If you do, you choose what to call them: WBS, phase, package, whatever you say
          on site.
        </div>
      ) : (
