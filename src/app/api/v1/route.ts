...
export async function GET(req: Request) {
  ...
  return jsonOk({
    ...
    endpoints: {
      ...
      "GET /api/v1/project-hours":         "Worked hours by project, one row per work session (last 12 months) with bill rate, billable amount and invoiced flag · filters: ?project_id=, ?account_id=, plus the enriched query layer (group_by=project.name&aggregates=sum:hours) · employee columns need the employees scope",
