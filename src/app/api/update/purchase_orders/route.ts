...
// Mirrors src/app/api/purchase-orders/[id]/route.ts PATCH's field-editable subset.
const ALLOWED = ["order_date", "expected_date", "notes", "terms", "custom_data"];
...
export async function POST(request: NextRequest) {
  ...
  const gate = await assertObjectFeature(tenantId, "purchase_orders");
  if (gate) return gate;
