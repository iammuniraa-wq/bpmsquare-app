...
// Subset of src/app/api/invoices/[id]/route.ts PATCH — discount_type/pct/fixed excluded
// (editable:false in FIELD_REGISTRY.invoice: they feed the stored total).
const ALLOWED = ["due_date", "notes", "terms", "custom_data"];
...
export async function POST(request: NextRequest) {
  ...
  const gate = await assertObjectFeature(tenantId, "invoices");
  if (gate) return gate;
  ...
  return NextResponse.json(await updateRows(supabase, "invoices", tenantId, prepared, outcomes, {
    objectType: "invoices", labelField: "ref", actorId: user?.id, actorEmail: user?.email,
  }));
}
