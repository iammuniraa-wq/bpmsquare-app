...
// Mirrors src/app/api/inventory/[id]/route.ts PATCH — supplier_id excluded (relationship
// changes aren't supported by bulk Update in v1; it isn't a mappable column anyway).
const ALLOWED = ["sku", "name", "description", "category", "uom", "reorder_level", "unit_cost", "notes", "status", "custom_data"];
...
export async function POST(request: NextRequest) {
  ...
  const gate = await assertObjectFeature(tenantId, "inventory");
  if (gate) return gate;
  ...
  return NextResponse.json(await updateRows(supabase, "inventory_items", tenantId, prepared, outcomes, {
    objectType: "inventory", labelField: "name", actorId: user?.id, actorEmail: user?.email,
  }));
}
