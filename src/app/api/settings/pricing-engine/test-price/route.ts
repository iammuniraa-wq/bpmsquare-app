...
// try a draft against a sample document BEFORE publishing it. Stored with
// source "test" so analysis (batch 6) can leave it out.
//
// { replay_of: <document id> } re-prices a stored document instead of the
// body's document -- "what would this real quote cost under my draft".
export async function POST(req: Request) {
  ...
  const replayOf = typeof body?.replay_of === "string" && body.replay_of ? body.replay_of : null;
  if (!replayOf) {
  ...
  const meta = { source: "test" as const, actorId: userId };
  try {
    const call = replayOf
      ? await replayPricingDocument(tenantId, replayOf, {
          configVersion: body?.options?.config_version, pricingDate: body?.options?.pricing_date, meta,
        })
      : await runPrice(
          tenantId,
    ...
    return NextResponse.json({ result, config_version, procedure, calc_ms, document_id, replay_of: replayOf });
  } catch (e) {
