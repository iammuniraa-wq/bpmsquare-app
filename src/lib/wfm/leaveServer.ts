...
// Server-side half of the leave type rules (0109): load a type with its
// limits (tolerating a database where 0109 is pending), and say whether a
// new span would break the type's monthly limit for this employee.
...
export async function loadLeaveType(admin: SupabaseClient, tenantId: string, id: string, activeOnly = false): Promise<LeaveTypeRow | null> {
  let q = admin.from("wfm_leave_types").select("id, name, category, active, monthly_limit, paid_days_per_month").eq("id", id).eq("tenant_id", tenantId);
  ...
  return { ...d, monthly_limit: d.monthly_limit ?? null, paid_days_per_month: d.paid_days_per_month ?? null };
}
...
export async function monthlyLimitError(
  ...
  if (!type.monthly_limit) return null;
  ...
  const v = checkMonthlyLimit(type.monthly_limit, span, existing);
  if (!v) return null;
