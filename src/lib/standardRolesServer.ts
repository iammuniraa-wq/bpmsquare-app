 ...
 * (tenant_id, template_key) -- which the migration makes uniquely indexed --
 * so a concurrent double-load can't create duplicates.
 *
...
export async function provisionStandardRoles(
  ...
  // A tenant may already have a hand-made role using a standard name --
  // business_roles has unique(tenant_id, name), so inserting a colliding
  // name would 23505 and lose the whole batch -- suffix instead of failing,
  // and let the admin tidy up if they want.
