...
//
// Each entry supplies just the glyph, sized via `size` (px) -- the tile and
// the modal header render the same badge family at two different scales, so
// the glyph itself has to scale with it rather than sit fixed-size and look
// lost in the larger one.
...
function GoogleCalendarGlyph({ size }: { size: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
function SlackGlyph({ size }: { size: number }) {
  return <span style={{ fontWeight: 800, fontSize: size, lineHeight: 1, fontFamily: "system-ui, sans-serif" }}>#</span>;
}

export const CONNECTOR_ICONS: Record<string, { Glyph: (props: { size: number }) => React.ReactNode; bg: string; fg: string }> = {
