-- 0065_standard_business_roles.sql
...
-- on first use -- not seeded here. See BUSINESS_ROLES_STANDARD_MAP.md §2 for
-- why: seeding rows in a migration needs a backfill for every new tenant and
-- leaves all 8 roles stale, with no safe update path, the moment a new
-- workcenter ships (`wfm` landed this month).
--
